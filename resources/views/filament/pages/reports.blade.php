<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <!-- P&L Card -->
            <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-500/10 via-purple-500/5 to-transparent border border-indigo-500/20 p-6 shadow-sm transition-all duration-300 hover:shadow-md hover:scale-[1.01]">
            ...
            <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500/10 via-teal-500/5 to-transparent border border-emerald-500/20 p-6 shadow-sm transition-all duration-300 hover:shadow-md hover:scale-[1.01]">
                <div class="absolute top-0 right-0 w-24 h-24 bg-emerald-500/10 rounded-full blur-2xl -mr-6 -mt-6"></div>
                <div class="flex items-center justify-between mb-4">
                    <span class="text-sm font-semibold tracking-wider text-emerald-400 uppercase">Portfolio Occupancy</span>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-baseline gap-2">
                        ...
                        <span class="text-xs text-emerald-400 font-semibold">{{ $occupancy['occupied'] }}/{{ $occupancy['total'] }} Units</span>
                    </div>
                    <!-- Custom sleek progress bar -->
                    <div class="w-full bg-gray-800 rounded-full h-1.5 mt-2">
                        <div class="bg-gradient-to-r from-emerald-400 to-teal-400 h-1.5 rounded-full" style="width: {{ $occupancy['rate'] }}%"></div>
                    </div>
                </div>
            ...
            <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500/10 via-orange-500/5 to-transparent border border-amber-500/20 p-6 shadow-sm transition-all duration-300 hover:shadow-md hover:scale-[1.01]">
                <div class="absolute top-0 right-0 w-24 h-24 bg-amber-500/10 rounded-full blur-2xl -mr-6 -mt-6"></div>
                <div class="flex items-center justify-between mb-4">
                    <span class="text-sm font-semibold tracking-wider text-amber-400 uppercase">Billing Operations</span>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-baseline gap-2">
                        <span class="text-3xl font-extrabold text-white tracking-tight">{{ $collection['efficiency'] }}%</span>
                        <span class="text-xs text-amber-400 font-semibold">₱{{ number_format($collection['outstanding'], 2) }} Outstanding</span>
                    </div>
                    <div class="w-full bg-gray-800 rounded-full h-1.5 mt-2">
                        <div class="bg-gradient-to-r from-amber-400 to-orange-400 h-1.5 rounded-full" style="width: {{ $collection['efficiency'] }}%"></div>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="w-full bg-gray-800 h-2 rounded-full overflow-hidden">
                            <div class="bg-amber-400 h-full rounded-full" style="width: {{ $aging['total'] > 0 ? ($aging['1_30'] / $aging['total'] * 100) : 0 }}%"></div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="w-full bg-gray-800 h-2 rounded-full overflow-hidden">
                            <div class="bg-orange-400 h-full rounded-full" style="width: {{ $aging['total'] > 0 ? ($aging['31_60'] / $aging['total'] * 100) : 0 }}%"></div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="w-full bg-gray-800 h-2 rounded-full overflow-hidden">
                            <div class="bg-rose-500 h-full rounded-full" style="width: {{ $aging['total'] > 0 ? ($aging['61_90'] / $aging['total'] * 100) : 0 }}%"></div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="w-full bg-gray-800 h-2 rounded-full overflow-hidden">
                            <div class="bg-red-600 h-full rounded-full" style="width: {{ $aging['total'] > 0 ? ($aging['91_plus'] / $aging['total'] * 100) : 0 }}%"></div>
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-2 bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-sm overflow-hidden flex flex-col">
                <h3 class="text-base font-bold text-white mb-4 flex items-center gap-2">
                        ...
                        <tbody class="divide-y divide-gray-800/50">
                            @forelse($agingDetails as $detail)
                                <tr class="hover:bg-gray-800/30 transition-colors">
                                    ...
                                    <td class="p-3 text-right font-bold text-rose-400">₱{{ number_format($detail['amount'], 2) }}</td>
                                    <td class="p-3 text-center">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide {{ $detail['days_overdue'] > 30 ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20' }}">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="p-6 text-center text-gray-500 font-medium">No aging receivables found. Great collection health!</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                    ...
                    <tbody class="divide-y divide-gray-800/50">
                        @forelse($unitsPerformance as $perf)
                            <tr class="hover:bg-gray-800/30 transition-colors">
                                ...
                                <td class="p-3">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold tracking-wide border {{ $perf['status'] === 'Occupied' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : ($perf['status'] === 'Vacant' ? 'bg-gray-500/10 text-gray-400 border-gray-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20') }}">
                                        {{ $perf['status'] }}
                                    </span>
                                </td>
                                <td class="p-3 text-right font-bold text-emerald-400">₱{{ number_format($perf['income'], 2) }}</td>
                                <td class="p-3 text-right font-bold text-rose-400">₱{{ number_format($perf['expenses'], 2) }}</td>
                                <td class="p-3 text-right font-semibold text-indigo-400">₱{{ number_format($perf['payouts'], 2) }}</td>
                                <td class="p-3 text-right font-black {{ $perf['net'] >= 0 ? 'text-emerald-400' : 'text-rose-500 bg-rose-500/5' }}">
                                    ₱{{ number_format($perf['net'], 2) }}
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="p-6 text-center text-gray-500 font-medium">No property performance details found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
